...
const COPY = {
  en: {
    title: 'Price comparisons', subtitle: 'Compare independent products and stores in one place.', newComparison: 'New comparison', selectComparison: 'Select a comparison', noComparisons: 'No price comparisons yet', noComparisonsHint: 'Create one to start adding stores, categories, products and prices.', setup: 'Setup', prices: 'Price matrix', report: 'Comparison report', stores: 'Stores', categories: 'Categories', products: 'Products', addStore: 'Add store', addCategory: 'Add category', addProduct: 'Add product', noStores: 'No stores have been added.', noCategories: 'No categories have been added.', noProducts: 'No products have been added.', optional: 'Optional', productCategories: 'Product categories', priceHint: 'Enter a price and save each cell. Empty cells do not have a recorded offer.', savePrice: 'Save price', deletePrice: 'Delete price', noMatrix: 'Add at least one store and one product to use the matrix.', loadingError: 'Could not load price comparison data.', saveError: 'Could not save the changes.', deleteError: 'Could not delete the item.', saved: 'Changes saved', deleted: 'Item deleted', requiredName: 'Name is required.', invalidPrice: 'Enter a valid non-negative price.', confirmDelete: 'Delete "{name}"? This action cannot be undone.', editItem: 'Edit {item}', addItem: 'Add {item}', comparison: 'comparison', store: 'store', category: 'category', product: 'product', refreshReport: 'Refresh report', reportEmpty: 'The report has no products yet.', minPrice: 'Minimum', maxPrice: 'Maximum', range: 'Range', cheapestOffers: 'Cheapest offer', noPrice: 'No price', productsByMinimum: 'Products by minimum price', storeRanking: 'Store basket ranking', basketTotal: 'Basket total', priced: 'priced', missing: 'missing', complete: 'Complete basket', incomplete: 'Incomplete basket', bestComplete: 'Best complete basket store', noCompleteBasket: 'No store has a complete basket.', reportSummary: 'Report summary', reportBackend: 'Calculated by the latest backend report.', editComparison: 'Edit comparison', deleteComparison: 'Delete comparison', loading: 'Loading...', description: 'Description', name: 'Name', cancel: 'Cancel', save: 'Save', edit: 'Edit', delete: 'Delete',
  },
  es: {
    title: 'Comparativas de precios', subtitle: 'Compara productos y tiendas independientes en un solo lugar.', newComparison: 'Nueva comparativa', selectComparison: 'Selecciona una comparativa', noComparisons: 'Aún no hay comparativas de precios', noComparisonsHint: 'Crea una para comenzar a agregar tiendas, categorías, productos y precios.', setup: 'Administración', prices: 'Matriz de precios', report: 'Reporte comparativo', stores: 'Tiendas', categories: 'Categorías', products: 'Productos', addStore: 'Agregar tienda', addCategory: 'Agregar categoría', addProduct: 'Agregar producto', noStores: 'No se han agregado tiendas.', noCategories: 'No se han agregado categorías.', noProducts: 'No se han agregado productos.', optional: 'Opcional', productCategories: 'Categorías del producto', priceHint: 'Introduce un precio y guarda cada celda. Las celdas vacías no tienen una oferta registrada.', savePrice: 'Guardar precio', deletePrice: 'Eliminar precio', noMatrix: 'Agrega al menos una tienda y un producto para usar la matriz.', loadingError: 'No se pudieron cargar los datos de la comparativa.', saveError: 'No se pudieron guardar los cambios.', deleteError: 'No se pudo eliminar el elemento.', saved: 'Cambios guardados', deleted: 'Elemento eliminado', requiredName: 'El nombre es obligatorio.', invalidPrice: 'Introduce un precio válido mayor o igual a cero.', confirmDelete: '¿Eliminar "{name}"? Esta acción no se puede deshacer.', editItem: 'Editar {item}', addItem: 'Agregar {item}', comparison: 'comparativa', store: 'tienda', category: 'categoría', product: 'producto', refreshReport: 'Actualizar reporte', reportEmpty: 'El reporte aún no tiene productos.', minPrice: 'Mínimo', maxPrice: 'Máximo', range: 'Rango', cheapestOffers: 'Oferta más barata', noPrice: 'Sin precio', productsByMinimum: 'Productos por precio mínimo', storeRanking: 'Ranking de canastas por tienda', basketTotal: 'Total de canasta', priced: 'con precio', missing: 'faltantes', complete: 'Canasta completa', incomplete: 'Canasta incompleta', bestComplete: 'Mejor tienda con canasta completa', noCompleteBasket: 'Ninguna tienda tiene una canasta completa.', reportSummary: 'Resumen del reporte', reportBackend: 'Calculado por el último reporte del backend.', editComparison: 'Editar comparativa', deleteComparison: 'Eliminar comparativa', loading: 'Cargando...', description: 'Descripción', name: 'Nombre', cancel: 'Cancelar', save: 'Guardar', edit: 'Editar', delete: 'Eliminar',
  },
} as const;
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, LoadingStateComponent, EmptyStateComponent, ErrorStateComponent, DialogFormDirective],
...
export class PriceComparisonDashboardComponent implements OnInit {
  ...
  tr(key: CopyKey, params?: Record<string, string>): string {
    ...
    for (const [name, replacement] of Object.entries(params ?? {})) result = result.replace(`{${name}}`, replacement);
    return result;
  }
  ...
  openEditor(type: EditorType, entity?: ManagedEntity): void {
    ...
    this.form = {
      name: entity?.name ?? '',
      description: 'description' in (entity ?? {}) ? String((entity as PriceComparison | ComparisonStore | ComparisonProduct).description) : '',
      categoryIds: type === 'product' && entity ? (entity as ComparisonProduct).categories.map((category) => category.id) : [],
    };
  }
  closeEditor(): void {
  ...
  toggleCategory(categoryId: string, checked: boolean): void {
    this.form.categoryIds = checked ? [...this.form.categoryIds, categoryId] : this.form.categoryIds.filter((id) => id !== categoryId);
  }
  saveEntity(): void {
    ...
    if (type === 'comparison') {
      ...
      const payload = { comparison: comparison!.id, name, description: this.form.description.trim() };
  ...
  priceFor(storeId: string, productId: string): ComparisonPrice | undefined {
    return this.prices().find((price) => price.storeId === storeId && price.productId === productId);
  }
  ...
  async deleteCellPrice(storeId: string, productId: string): Promise<void> {
    const existing = this.priceFor(storeId, productId);
    if (!existing) return;
    const confirmed = await this.confirm.confirm(this.tr('confirmDelete', { name: `${existing.price}` }), { variant: 'danger', confirmLabel: this.tr('delete') });
    if (!confirmed) return;
    const key = this.cellKey(storeId, productId);
    this.setCellSaving(key, true);
